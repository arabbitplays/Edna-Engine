...
namespace mandelbrot
{
    ...
    void MandelbrotAnimationRunner::update(const glm::vec2& view_center, float view_span, bool julia_mode)
    {
        ...
        if (view_span > 0.0F)
        {
            const ProbeResult probe =
                probeInterest(static_cast<double>(view_center.x), static_cast<double>(view_center.y),
                    static_cast<double>(view_span), static_cast<double>(current_state_.initial.x),
                    static_cast<double>(current_state_.initial.y), julia_mode,
                    MandelbrotAnimationGenerator::PROBE_MAX_ITER, MandelbrotAnimationGenerator::PROBE_GRID_SIZE);
    ...
    void MandelbrotAnimationRunner::tick(
        Track& track, float dt, float speed, void (MandelbrotAnimationRunner::*start)())
    {
        if (track.animation)
        {
