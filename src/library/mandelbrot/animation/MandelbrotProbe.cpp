...
{
    ProbeResult probeInterest(const double center_re, const double center_im, const double span,
        const double initial_re, const double initial_im, const bool julia_mode, const std::uint32_t max_iter,
        const std::uint32_t grid_size)
    {
        if (grid_size == 0U || max_iter == 0U)
        {
            return {.edge_score = 0.0F, .inside_fraction = 0.0F, .mu_norm = {}, .grid_size = 0U};
        }
        ...
        for (std::uint32_t j = 0; j < grid_size; ++j)
        {
            ...
            for (std::uint32_t i = 0; i < grid_size; ++i)
            {
                ...
                mu_norm[idx] = static_cast<float>(std::clamp(mu / static_cast<double>(max_iter), 0.0, 1.0));
            }
        }

        const float inside_fraction = static_cast<float>(inside_count) / static_cast<float>(pixel_count);
        ...
        for (std::uint32_t j = 0; j < grid_size; ++j)
        {
            for (std::uint32_t i = 0; i < grid_size; ++i)
            {
                ...
                if (j + 1U < grid_size)
                {
                    if (is_boundary(idx, idx + grid_size))
                    {
                        ++boundary_pairs;
                    }
                    ++total_pairs;
        ...
        const float edge_score =
            total_pairs == 0U ? 0.0F : static_cast<float>(boundary_pairs) / static_cast<float>(total_pairs);

        return {.edge_score = edge_score,
            .inside_fraction = inside_fraction,
            .mu_norm = std::move(mu_norm),
            .grid_size = grid_size};
    }
