...
namespace Animation
{
    template <typename T, typename AnimT> AnimationCurve<T, AnimT>& AnimationCurve<T, AnimT>::add(FixPoint point)
    {
    ...
    std::vector<std::unique_ptr<Animation<T>>> AnimationCurve<T, AnimT>::build() const
    {
        ...
        for (size_t i = 1; i < points.size(); ++i)
        {
            ...
            auto easing = cur.easing_override ? cur.easing_override : makeEasingFunction(cur.curve, cur.direction);

            segments.emplace_back(
                std::make_unique<AnimT>(prev.value, cur.value, cur.steps, on_update, std::move(easing)));
        }
        return segments;
    }
