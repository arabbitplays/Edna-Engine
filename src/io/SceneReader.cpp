...
namespace RtEngine
{
    ...
    std::shared_ptr<Scene> SceneReader::readScene(
        const std::string& file_path, std::unordered_map<std::string, std::shared_ptr<Material>> materials)
    {
        ...
        try
        {
            ...
            std::shared_ptr<Scene> scene = std::make_shared<Scene>(file_path, scene_material);
    ...
    void SceneReader::loadSceneLights(const YAML::Node& lights_node, std::shared_ptr<Scene>& scene)
    {
        ...
        {
            scene->pointLights[point_light_index++] = PointLight(point_light_node["position"].as<glm::vec3>(),
                point_light_node["color"].as<glm::vec3>(), point_light_node["intensity"].as<float>());
        }
    }
    ...
    std::shared_ptr<Node> SceneReader::processSceneNodesRecursiv(
    ...
    void SceneReader::readComponents(const YAML::Node& yaml_node, std::shared_ptr<Node>& scene_node)
    {
        ...
        for (const auto& comp_node : yaml_node["components"])
        {
            ...
            {
                std::shared_ptr<MeshRenderer> mesh_component = std::make_shared<MeshRenderer>(ctx, scene_node);
