...
namespace RtEngine
{
    Runner::Runner(
        ...
        : engine_context(engine_context),
          ...
          sync_manager(engine_context->sync_manager), scene_manager(scene_manager)
    {
    ...
    {
        std::shared_ptr<Scene> new_scene = scene_manager->loadScene(scene_path, raytracing_renderer->getMaterials());
    ...
    void Runner::renderFrame(uint32_t frame_idx, uint32_t swapchain_image_idx, bool present) const
    {
        ...
        for (size_t i = 0; i < renderers.size(); i++)
        {
            ...
            sync_manager->submitStage(
                static_cast<uint32_t>(i), vulkan_context->device_manager->getQueue(renderers[i]->queueType()), cmd);
        }
