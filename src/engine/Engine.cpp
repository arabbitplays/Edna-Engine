...
namespace RtEngine
{
    ...
    void Engine::createGuiManager()
    {
        ...
        gui_manager->addCallbackToAll(
            [&](const UpdateFlagsHandle& update_flags) { runner->setUpdateFlags(update_flags); });
    }
    ...
    void Engine::createEngineContext()
    {
        ...
        scene_manager = std::make_shared<SceneManager>(options->resources_dir,
            rendering_manager->getVulkanContext()->device_manager, scene_reader); // non-interfaced version
    ...
    void Engine::parseCliArguments(CliArguments cli_args)
    {
        ...
        cli_parser.addString(
            "--resources", &options->resources_dir, "The path to the directory where all resource files can be found.");
