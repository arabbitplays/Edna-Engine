...
namespace RtEngine
{
    PresentStage::PresentStage(std::shared_ptr<VulkanContext> vulkan_context, std::shared_ptr<SyncManager> sync_manager,
        std::shared_ptr<GuiRenderer> gui_renderer, const uint32_t max_frames_in_flight)
        : vulkan_context(std::move(vulkan_context)), sync_manager(std::move(sync_manager)),
          gui_renderer(std::move(gui_renderer)), max_frames_in_flight(max_frames_in_flight)
    {
    }
    ...
    int32_t PresentStage::acquireNextSwapchainImage()
    {
        ...
        VkResult result =
            vkAcquireNextImageKHR(vulkan_context->device_manager->getDevice(), vulkan_context->swapchain->handle,
                UINT64_MAX, sync_manager->imageAvailableSemaphore(), VK_NULL_HANDLE, &image_index);
    ...
    bool PresentStage::submitAndPresent(
        const uint32_t stage_index, const std::shared_ptr<ImageConnector>& source, const uint32_t swapchain_image_idx)
    {
        ...
        sync_manager->submitStage(stage_index, vulkan_context->device_manager->getQueue(GRAPHICS), cmd,
    ...
    void PresentStage::recordBlit(
        VkCommandBuffer cmd, AllocatedImage source_image, VkExtent2D source_extent, const uint32_t swapchain_image_idx)
    {
        ...
        resource_builder->transitionImageLayout(cmd, swapchain->images[swapchain_image_idx],
            VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_NONE,
        ...
        resource_builder->transitionImageLayout(cmd, source_image.image, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
            VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_NONE, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
        ...
        vkCmdBlitImage(cmd, source_image.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            swapchain->images[swapchain_image_idx], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit_region,
            VK_FILTER_NEAREST);
    }
    ...
    void PresentStage::transitionSwapchainForPresent(VkCommandBuffer cmd, const uint32_t swapchain_image_idx)
    {
        ...
        vulkan_context->resource_builder->transitionImageLayout(cmd,
            vulkan_context->swapchain->images[swapchain_image_idx], VK_PIPELINE_STAGE_TRANSFER_BIT,
            VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_NONE,
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
    }

    void PresentStage::transitionSourceBackToGeneral(VkCommandBuffer cmd, AllocatedImage source_image)
    {
        vulkan_context->resource_builder->transitionImageLayout(cmd, source_image.image, VK_PIPELINE_STAGE_TRANSFER_BIT,
            VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_NONE,
