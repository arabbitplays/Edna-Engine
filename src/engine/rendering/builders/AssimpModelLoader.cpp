...
namespace RtEngine
{
    ...
    void AssimpModelLoader::processNode(
        aiNode* node, const aiScene* scene, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices)
    {
    ...
    void AssimpModelLoader::processMesh(
        aiMesh* mesh, const aiScene* scene, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices)
    {
