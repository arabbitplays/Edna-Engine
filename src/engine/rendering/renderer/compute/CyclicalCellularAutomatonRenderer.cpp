...
namespace RtEngine
{
    CyclicalCellularAutomatonRenderer::CyclicalCellularAutomatonRenderer(
        const std::shared_ptr<VulkanContext>& vulkan_context, VkExtent2D image_extent,
        const std::vector<glm::vec4>& colors, const std::vector<glm::ivec2>& neighbor_offsets,
        const uint32_t max_frames_in_flight)
        : ComputeRenderer(vulkan_context, max_frames_in_flight), image_extent(image_extent), colors(colors)
    {
        assert(!colors.empty() && "CyclicalCellularAutomatonRenderer: colors must not be empty");
        assert(
            colors.size() <= MAX_STATE_COUNT &&
            "CyclicalCellularAutomatonRenderer: colors.size() must not exceed MAX_STATE_COUNT (palette UBO capacity)");
        assert(!neighbor_offsets.empty() && "CyclicalCellularAutomatonRenderer: neighbor_offsets must not be empty");
        assert(neighbor_offsets.size() <= MAX_NEIGHBOR_COUNT &&
               "CyclicalCellularAutomatonRenderer: neighbor_offsets.size() must not exceed MAX_NEIGHBOR_COUNT "
               "(neighborhood UBO capacity)");

        push.state_count = static_cast<uint32_t>(colors.size());

        state_connector =
            std::make_shared<ImageConnector>(vulkan_context->resource_builder, image_extent, 1, VK_FORMAT_R32_UINT,
        ...
        rng_connector =
            ImageConnectorFactory::createRngTextureConnector(vulkan_context->resource_builder, image_extent, 1);
        ...
        neighborhood_connector = BufferConnectorFactory::createUniformBuffer(
            vulkan_context->resource_builder, vulkan_context->device_manager, neighborhoodBufferSize(), 1);
    ...
    {
        assert(!new_offsets.empty() && "CyclicalCellularAutomatonRenderer::setNeighborhood: offsets must not be empty");
    ...
    {
        assert(!new_colors.empty() && "CyclicalCellularAutomatonRenderer::setPalette: colors must not be empty");
    ...
    VkShaderModule CyclicalCellularAutomatonRenderer::createShaderModule()
    {
        return VulkanUtil::createShaderModule(vulkan_context->device_manager->getDevice(),
            oschd_cyclical_cellular_automaton_comp_spv_size(), oschd_cyclical_cellular_automaton_comp_spv());
    }
