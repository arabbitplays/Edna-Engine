...
namespace RtEngine
{
        ...
        VkPhysicalDeviceRayTracingPipelinePropertiesKHR raytracing_properties)
    {
        ...
        const uint32_t sbt_usage_flags =
        ...
        raygenShaderBindingTable = context->resource_builder->createBuffer(
            rgen_indices.size() * handle_size_aligned, sbt_usage_flags, sbt_memory_property_flags);
        missShaderBindingTable = context->resource_builder->createBuffer(
            miss_indices.size() * handle_size_aligned, sbt_usage_flags, sbt_memory_property_flags);
        hitShaderBindingTable = context->resource_builder->createBuffer(
            hit_indices.size() * handle_size_aligned, sbt_usage_flags, sbt_memory_property_flags);
        deletionQueue.pushFunction(
            [&]()
            {
        ...
        if (getRayTracingShaderGroupHandlesKhr(
                device, handle, 0, group_count, sbt_size, shader_handle_storage.data()) != VK_SUCCESS)
        {
        ...
        auto copy_handle = [&](AllocatedBuffer& buffer, std::vector<uint32_t>& indices, uint32_t stride)
        {
            ...
            {
                buffer.update(
                    device, shader_handle_storage.data() + (indice * handle_size_aligned), handle_size, offset);
    ...
    void RaytracingPipeline::addShaderStage(
        VkShaderModule shader_module, VkShaderStageFlagBits shader_stage, VkRayTracingShaderGroupTypeKHR shader_group)
    {
    ...
    VkPipelineLayout RaytracingPipeline::getLayoutHandle() const
    {
        return layout;
    }

    uint32_t RaytracingPipeline::getGroupCount() const
    {
