...
namespace RtEngine
{
    ...
    void MetalRoughMaterial::buildPipelines(VkDescriptorSetLayout scene_layout)
    {
        ...
        layout_builder.addBinding(
            1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 64); // TODO make this dynamic depending on the scene
        ...
        VkShaderModule shadow_miss_shader_module =
            VulkanUtil::createShaderModule(device, oschd_shadow_miss_rmiss_spv_size(), oschd_shadow_miss_rmiss_spv());
        ...
        pipeline->addShaderStage(
            raygen_shader_module, VK_SHADER_STAGE_RAYGEN_BIT_KHR, VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR);
        pipeline->addShaderStage(
            miss_shader_module, VK_SHADER_STAGE_MISS_BIT_KHR, VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR);
        pipeline->addShaderStage(
            shadow_miss_shader_module, VK_SHADER_STAGE_MISS_BIT_KHR, VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR);
    ...
    void MetalRoughMaterial::writeMaterial(
        AllocatedBuffer material_buffer, std::shared_ptr<MaterialTextures<>> material_textures)
    {
        descriptorAllocator.writeBuffer(0, material_buffer.handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);

        descriptorAllocator.writeImages(1, material_textures->getOrderedImageViews(), sampler,
