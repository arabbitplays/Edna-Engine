...
namespace RtEngine {
	...
	bool DeviceManager::isDeviceSuitable(VkPhysicalDevice device) {
		...
		return extensions_supported && indices.isComplete() && swap_chain_adequate && (device_features.samplerAnisotropy != 0u) &&
			   (device_features.shaderInt64 != 0u) && (device_features.shaderFloat64 != 0u) &&
			   (raytracing_pipeline_features.rayTracingPipeline != 0u) && (acceleration_structure_features.accelerationStructure != 0u) &&
	...
	void DeviceManager::createLogicalDevice(bool enable_validation_layers) {
		...
		std::set<uint32_t> unique_queue_families = {queue_indices.graphicsAndComputeFamily.value(),
