...
namespace RtEngine
{
    ...
    VkDescriptorPool DescriptorAllocator::createPool(
        const VkDevice device, const uint32_t set_count, std::span<DescriptorAllocator::PoolSizeRatio> pool_ratios)
    {
        std::vector<VkDescriptorPoolSize> pool_sizes;
        for (auto [type, ratio] : pool_ratios)
        {
            VkDescriptorPoolSize pool_size{
                .type = type, .descriptorCount = static_cast<uint32_t>(ratio * static_cast<float>(set_count))};
    ...
    VkDescriptorPool DescriptorAllocator::createPool(const VkDevice device,
        const std::vector<VkDescriptorPoolSize>& pool_sizes, const VkDescriptorPoolCreateFlags flags)
    {
    ...
    VkDescriptorSet DescriptorAllocator::allocate(
        const VkDevice device, const VkDescriptorSetLayout layout, const void* p_next)
    {
    ...
    void DescriptorAllocator::writeBuffer(
        uint32_t binding, VkBuffer buffer, VkDeviceSize size, uint32_t offset, VkDescriptorType type)
    {
    ...
    void DescriptorAllocator::writeImage(
        uint32_t binding, VkImageView image_view, VkSampler sampler, VkImageLayout layout, VkDescriptorType type)
    {
    ...
    void DescriptorAllocator::writeImages(const uint32_t binding, const std::vector<VkImageView>& image_views,
        const VkSampler sampler, const VkImageLayout layout, const VkDescriptorType type)
    {
        ImageInfoWrapper wrapper{};
        wrapper.image_infos.resize(image_views.size());
        for (size_t i = 0; i < image_views.size(); i++)
        {
            wrapper.image_infos[i] =
                VkDescriptorImageInfo{.sampler = sampler, .imageView = image_views[i], .imageLayout = layout};
        }
        imageInfos.push_back(wrapper);
    ...
    void DescriptorAllocator::writeAccelerationStructure(
        uint32_t binding, VkAccelerationStructureKHR acceleration_structure, VkDescriptorType type)
    {
