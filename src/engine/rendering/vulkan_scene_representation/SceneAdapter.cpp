...
namespace RtEngine
{
    ...
    void SceneAdapter::createTlas()
    {
        ...
        top_level_acceleration_structure =
            std::make_shared<AccelerationStructure>(device, *vulkan_context->resource_builder,
                *vulkan_context->command_manager, VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR);
    ...
    void SceneAdapter::createSceneLayout()
    {
        ...
        scene_descriptor_set_layout = layout_builder.build(vulkan_context->device_manager->getDevice(),
            VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR);
        main_deletion_queue.pushFunction(
            [&]()
            {
                vkDestroyDescriptorSetLayout(
                    vulkan_context->device_manager->getDevice(), scene_descriptor_set_layout, nullptr);
            });
    }
    ...
    void SceneAdapter::createSceneDescriptorSets(const VkDescriptorSetLayout& layout)
    {
        ...
        {
            scene_descriptor_sets.push_back(
                vulkan_context->descriptor_allocator->allocate(vulkan_context->device_manager->getDevice(), layout));
        }
    }
    ...
    void SceneAdapter::createUniformBuffers(const std::shared_ptr<IScene>& scene)
    {
        ...
        for (size_t i = 0; i < max_frames_in_flight; i++)
        {
            ...
            sceneUniformBuffers[i] =
                vulkan_context->resource_builder->createBuffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
    ...
    void SceneAdapter::updateScene(
        const std::shared_ptr<DrawContext>& draw_context, uint32_t current_frame, const UpdateFlagsHandle& update_flags)
    {
    ...
    void SceneAdapter::updateRenderTarget(const std::shared_ptr<RenderTarget>& target)
    {
        ...
        vulkan_context->descriptor_allocator->writeImage(9, target->getCurrentRngImage().imageView, VK_NULL_HANDLE,
            VK_IMAGE_LAYOUT_GENERAL, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    ...
    void SceneAdapter::updateStaticGeometry(
        std::vector<RenderObject> render_objects, const UpdateFlagsHandle& update_flags)
    {
        if (update_flags->checkFlag(STATIC_GEOMETRY_UPDATE))
        {
            ...
            vulkan_context->descriptor_allocator->writeBuffer(
                6, instance_manager->getInstanceBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        ...
        if (update_flags->checkFlag(STATIC_GEOMETRY_UPDATE) || update_flags->checkFlag(MATERIAL_UPDATE))
        {
            ...
            vulkan_context->descriptor_allocator->writeBuffer(
                7, instance_manager->getEmittingInstancesBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        }
    ...
    void SceneAdapter::updateTlas(std::vector<RenderObject> objects) const
    {
        ...
        {
            top_level_acceleration_structure->addInstance(
                object.acceleration_structure, object.transform, instance_id++);
        }
    ...
    void SceneAdapter::updateSceneData(const std::shared_ptr<IScene>& scene,
        const std::shared_ptr<DrawContext>& draw_context, uint32_t current_frame) const
    {
        size_t size = 0;
        ...
        vulkan_context->descriptor_allocator->writeBuffer(
            2, sceneUniformBuffers[current_frame].handle, size, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    ...
    void SceneAdapter::initDefaultResources(
    ...
    void SceneAdapter::createDefaultMaterials(
        const VkPhysicalDeviceRayTracingPipelinePropertiesKHR& raytracing_properties)
    {
