...
namespace RtEngine
{
    ...
    void GeometryManager::writeGeometryBuffers() const
    {
        vulkan_context->descriptor_allocator->writeBuffer(
            3, vertex_buffer.handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(4, index_buffer.handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(
            5, geometry_mapping_buffer.handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
    }
    ...
    AllocatedBuffer GeometryManager::createVertexBuffer(std::vector<std::shared_ptr<MeshAsset>>& mesh_assets) const
    {
        ...
        return vulkan_context->resource_builder->stageMemoryToNewBuffer(vertices.data(),
    ...
    AllocatedBuffer GeometryManager::createIndexBuffer(std::vector<std::shared_ptr<MeshAsset>>& mesh_assets) const
    {
        ...
        return vulkan_context->resource_builder->stageMemoryToNewBuffer(indices.data(),
            indices.size() * sizeof(uint32_t),
            VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    }

    AllocatedBuffer GeometryManager::createGeometryMappingBuffer(
        std::vector<std::shared_ptr<MeshAsset>>& mesh_assets) const
    {
        assert(!mesh_assets.empty());
    ...
    void GeometryManager::createBlas(std::vector<std::shared_ptr<MeshAsset>>& meshes)
    {
        ...
        {
            mesh_asset->accelerationStructure =
                std::make_shared<AccelerationStructure>(device, *vulkan_context->resource_builder,
                    *vulkan_context->command_manager, VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR);

            mesh_asset->accelerationStructure->addTriangleGeometry(vertex_buffer, index_buffer,
