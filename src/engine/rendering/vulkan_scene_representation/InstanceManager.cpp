...
namespace RtEngine
{
    void InstanceManager::createInstanceMappingBuffer(std::vector<RenderObject>& objects)
    {
        ...
        instance_mapping_buffer = resource_builder->stageMemoryToNewBuffer(instance_datas.data(),
            instance_datas.size() * sizeof(InstanceMappingData), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    }
    ...
    void InstanceManager::createEmittingInstancesBuffer(const std::vector<RenderObject>& objects)
    {
        ...
        emitting_instances_buffer = resource_builder->stageMemoryToNewBuffer(emitting_instances.data(),
            emitting_instances.size() * sizeof(EmittingInstanceData), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    }
