...
namespace RtEngine
{
    MaterialManager::MaterialManager(
        ...
        : resource_builder(std::move(std::move(resource_builder)))
    {
    ...
    void MaterialManager::updateMaterialResources(const std::shared_ptr<IScene>& scene)
    {
        ...
        // collect material instance from scene graph, combine their data resources into a buffer and the textures into
        // material_textures
    ...
    AllocatedBuffer MaterialManager::createMaterialBuffer(
        const std::vector<std::shared_ptr<MaterialInstance>>& instances) const
    {
        ...
        AllocatedBuffer material_buffer =
            resource_builder->stageMemoryToNewBuffer(material_data, total_size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
