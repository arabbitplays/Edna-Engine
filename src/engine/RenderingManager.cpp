...
namespace RtEngine
{
    RenderingManager::RenderingManager(const std::shared_ptr<Window>& window, std::string resources_dir,
        const bool enable_validation_layer, const bool enable_raytracing)
        : window(window), validation_layers_enabled(enable_validation_layer), enable_raytracing(enable_raytracing),
          resources_dir(std::move(std::move(resources_dir)))
    {

        createVulkanContext();
        createRenderer();

        window->addResizeCallback([this](uint32_t width, uint32_t height) { framebufferResized = true; });
    }

    void RenderingManager::initRendererProperties(
        const std::shared_ptr<IProperties>& properties, const std::shared_ptr<UpdateFlags>& update_flags)
    {
    ...
    void RenderingManager::createVulkanContext()
    {
        ...
        vulkan_context->device_manager =
            std::make_shared<DeviceManager>(window->getHandle(), validation_layers_enabled);

        vulkan_context->command_manager = std::make_shared<CommandManager>(vulkan_context->device_manager);
        vulkan_context->resource_builder = std::make_shared<ResourceBuilder>(
            vulkan_context->device_manager, vulkan_context->command_manager, resources_dir);
        vulkan_context->swapchain = std::make_shared<Swapchain>(
            vulkan_context->device_manager, window->getHandle(), vulkan_context->resource_builder);
        ...
        sync_manager =
            std::make_shared<SyncManager>(max_frames_in_flight, vulkan_context->device_manager, swapchain_manager);
    ...
    void RenderingManager::createRenderer()
    {
        ...
        present_stage =
            std::make_shared<PresentStage>(vulkan_context, sync_manager, gui_renderer, max_frames_in_flight);
        present_stage->init();

        createRepositories();

        RendererStackFactory factory(
            vulkan_context, present_stage, mesh_repository, texture_repository, max_frames_in_flight);
    ...
    void RenderingManager::addComputeRenderer(
        const std::shared_ptr<ComputeRenderer>& renderer, std::shared_ptr<ImageConnector> new_present_connector)
    {
