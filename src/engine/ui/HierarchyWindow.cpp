...
namespace RtEngine
{
    HierarchyWindow::HierarchyWindow(
        const std::shared_ptr<InspectorWindow>& inspector_window, const std::shared_ptr<SceneManager>& scene_manager)
    ...
    void HierarchyWindow::createFrame()
    {
        ...
        for (auto& node_add : nodes_to_add)
        {
            ...
            scene->nodes[node_add.node_key]->transform->setLocalTransform(
                glm::inverse(new_parent_transform) * prev_world_transform);
        }
        ...
        for (auto& node_remove : nodes_to_remove)
        {
            scene->nodes[node_remove.parent_key]->children.erase(
                std::remove(scene->nodes[node_remove.parent_key]->children.begin(),
                    scene->nodes[node_remove.parent_key]->children.end(), scene->nodes[node_remove.node_key]),
    ...
    void HierarchyWindow::displayNode(
        const std::shared_ptr<Node>& node, const std::shared_ptr<Node>& parent, uint32_t depth)
    {
