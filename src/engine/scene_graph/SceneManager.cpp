...
namespace RtEngine
{
    SceneManager::SceneManager(const std::string& resources_dir, const std::shared_ptr<DeviceManager>& device_manager,
        std::shared_ptr<SceneReader> scene_reader)
        : resources_dir(resources_dir), device_manager(device_manager), scene_reader(std::move(scene_reader))
    {
    ...
    std::shared_ptr<Scene> SceneManager::loadScene(
