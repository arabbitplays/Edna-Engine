...
namespace RtEngine
{
    ...
    bool YamlDumpProperties::addUint(const std::string& name, uint32_t* var, uint32_t min, uint32_t max, uint32_t flags)
    {
        nodes.back()[name] = *var;
        return true;
    }
    ...
    bool YamlDumpProperties::addSelection(
        const std::string& name, std::string* var, std::vector<std::string> selection_options, uint32_t flags)
    {
        nodes.back()[name] = *var;
        return true;
    }
