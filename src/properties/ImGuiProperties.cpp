...
namespace RtEngine
{
    ...
    bool ImGuiProperties::addSelection(
        const std::string& name, std::string* var, std::vector<std::string> options, uint32_t flags)
    {
        assert(!options.empty());

        std::vector<const char*> options_c_str;
        std::ranges::transform(options, std::back_inserter(options_c_str), [](auto& str) { return str.c_str(); });
