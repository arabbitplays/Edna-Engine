...
namespace RtEngine
{
    ...
    public:
        ...
        RenderingManager(const std::shared_ptr<Window>& window, std::string resources_dir, bool enable_validation_layer,
            bool enable_raytracing = true);

        void initRendererProperties(
            const std::shared_ptr<IProperties>& properties, const std::shared_ptr<UpdateFlags>& update_flags);

        bool raytracingEnabled() const
        {
            return enable_raytracing;
        }
