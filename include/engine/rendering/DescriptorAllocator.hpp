...
namespace RtEngine
{
    ...
    public:
        ...
        static VkDescriptorPool createPool(
            VkDevice device, const std::vector<VkDescriptorPoolSize>& pool_sizes, VkDescriptorPoolCreateFlags flags);
        ...
        void writeImages(uint32_t binding, const std::vector<VkImageView>& imageViews, VkSampler sampler,
            VkImageLayout layout, VkDescriptorType type);
        void writeAccelerationStructure(
            uint32_t binding, VkAccelerationStructureKHR accelerationStructure, VkDescriptorType type);
