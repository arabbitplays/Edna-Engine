...
namespace RtEngine
{
    ...
    {
    public:
        GlitchRenderer(const std::shared_ptr<VulkanContext>& vulkan_context, VkExtent2D image_extent,
            std::shared_ptr<ImageConnector> input_connector, uint32_t max_frames_in_flight = 1);

        std::shared_ptr<ImageConnector> getOutputConnector() const;

        void handleResize(VkExtent2D new_extent);

        void setShakePower(float v)
        {
            push.shake_power = v;
        }
        void setShakeRate(float v)
        {
            push.shake_rate = v;
        }
        void setShakeSpeed(float v)
        {
            push.shake_speed = v;
        }
        void setShakeBlockSize(float v)
        {
            push.shake_block_size = v;
        }
        void setShakeColorRate(float v)
        {
            push.shake_color_rate = v;
        }
